...
import { fetchInfoLokerPerusahaanById, updateInfoLokerPerusahaan } from '../../../../redux/slice/infoLokerPerusahaanSlice';
import { FaExclamationCircle, FaPlus, FaSave, FaTimes } from 'react-icons/fa';
import Cropper from 'cropperjs';
import { FiAlertTriangle } from 'react-icons/fi';

export default function UpdateInfoLokerPerusahaan() {
    const { id } = useParams();
    ...
    const { loading, error, infoLokerPerusahaan } = useSelector((state) => state.infoLokerPerusahaan);
    ...
    // Populate form data when job information is fetched
    useEffect(() => {
        if (infoLokerPerusahaan && Object.keys(infoLokerPerusahaan).length > 0) {
            const foundLoker = infoLokerPerusahaan;
            setFormData({
                ...
                persyaratan: Array.isArray(foundLoker.persyaratan) ? foundLoker.persyaratan.join(', ') : foundLoker.persyaratan || '',
                keterampilan: Array.isArray(foundLoker.keterampilan) ? foundLoker.keterampilan.join(', ') : foundLoker.keterampilan || '',
                gaji_min: foundLoker.gaji_min || '',
                gaji_max: foundLoker.gaji_max || '',
                poster: foundLoker.poster || null,
                logo: foundLoker.logo || null
            });
            setIsGajiSecret(foundLoker.gaji_min === "Dirahasiakan" && foundLoker.gaji_max === "Dirahasiakan");
        }
    }, [infoLokerPerusahaan, id]);
    ...
    // Handle salary secrecy checkbox change
    ...
    const confirmSubmit = async () => {
        ...
        try {
            ...
            await dispatch(updateInfoLokerPerusahaan({ id, formData: formDataToSend, userId })).unwrap();
    ...
    return (
        <PageContainer title="Edit Info Lowongan Kerja">
            ...
            <div className="d-flex flex-column flex-md-row justify-content-between align-items-center align-items-md-start mb-3">
                <h4 className="fw-bold mb-3 mb-md-0">Edit Info Lowongan Kerja</h4>
                ...
                <div className="p-0 m-0 w-100" style={{ maxWidth: "500px" }}>
                    <Modal.Body className="d-flex flex-column align-items-center text-center pb-0">
                        ...
                        <p className="text-muted" style={{ fontSize: 'clamp(0.875rem, 3vw, 1rem)' }}>
                            Apakah Anda yakin perubahan data lowongan ini sudah sesuai?
                        </p>
                    </Modal.Body>
                    <Modal.Footer 
